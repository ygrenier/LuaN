    ...
    return a, b, c
        ...
        public void TestLuaDebugHook()
        {
            using (ILuaState L = new LuaState())
            {
                ...
                {
                    state.LuaGetInfo(LuaGetInfoWhat.AllFills, dg);
                    output.Add(String.Format("Hook ({0}) Line {1} : {2} ({3}). {4}.", dg.Event, dg.CurrentLine, dg.What, dg.NameWhat, dg.Name));
                };

                L.LuaSetHook(hook, LuaHookMask.MaskCall | LuaHookMask.MaskLine | LuaHookMask.MaskRet, 0);
...
");
        ...
        [Fact]
        public void TestLuaGetDebugInfo()
        {
            using (ILuaState L = new LuaState())
            {
                List<String> output = new List<string>();
...
");
                ...
                {
                    L.LuaGetInfo(LuaGetInfoWhat.AllFills, ar);
                    if (ar.Name == "test")
                    {
                        // Locales
                        for (int i = -2; i < 5; i++)
                        {
                            if (i == 0) continue;
                            var s = L.LuaGetLocal(ar, i);
                            output.Add(String.Format("Local #{0} = {1}", i, s));
                            if (s != null)
                                L.LuaPop(1);
                        }
                        // Change local 2 (b)
                        Assert.Equal("b", L.LuaGetLocal(ar, 2));
                        L.LuaPushNumber(7);
                        L.LuaArith(LuaArithOperator.Add);
                        Assert.Equal("b", L.LuaSetLocal(ar, 2));
                        // Trace
                        int level = 0;
                        using (var tar = state.NewLuaDebug())
                        {
                            while (state.LuaGetStack(level++, tar))
                            {
                                state.LuaGetInfo(LuaGetInfoWhat.AllFills, tar);
                ...
                {
                    L.LuaGetGlobal("test");
                    Assert.True(L.LuaGetInfo(LuaGetInfoWhat.FromTopOfStack | LuaGetInfoWhat.AllFills, ar));
